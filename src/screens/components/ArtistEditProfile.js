...
export default class ArtistEditProfile extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      name: '',
    ...
    this.artistEditProfile = this.artistEditProfile.bind(this);
    this.artistEditProfileByID = this.artistEditProfileByID.bind(this);
  }
  ...
  artistEditProfile() {
    const { pro_id, name, region, fullBio, bioShortIntroduction, twitter, facebook, youtube, soundCloud, ImageSource } = this.state;
    ...
    if (ImageSource == "" || ImageSource == null || ImageSource == undefined) {
      ...
      formData.append('pro_title', name);
      formData.append('pro_region', region);
      formData.append('pro_intro', fullBio);
      formData.append('pro_content1', bioShortIntroduction);
      formData.append('pro_tweets', twitter);
      formData.append('pro_instagram', facebook);
      formData.append('pro_youtube', youtube);
      ...
      console.log("Formdata parametres",formData);
      fetch(appConstant.BASE_URL + appConstant.ARTISTEDITPROFILE, {
        method: 'POST',
        headers: {
        ...
        .catch((error) => {
          this.setState({ loading: false });
          console.log("Artist Edit Profile  error: ", error);
        });
    }

  }
  artistEditProfileByID() {
    this.setState({ loading: true });
    fetch(appConstant.BASE_URL + appConstant.GETPROJECT + '?pro_id=' + this.state.pro_id, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'multipart/form-data',
      ...
      if (this.isJson(response._bodyInit)) {
        ...
        console.log("Sucessfully get by id artist...", responseJson);
        this.setState({
          loading: false,
          artistDetailById: responseJson.results,
          name: responseJson.results.pro_title,
          region: responseJson.results.pro_region,
          bioShortIntroduction: responseJson.results.pro_intro,
          fullBio: responseJson.results.pro_content1,
          twitter: responseJson.results.pro_tweets,
          facebook: responseJson.results.pro_instagram,
          youtube: responseJson.results.pro_youtube,
          soundCloud: responseJson.results.pro_soundcloud,
          ImageSource: appConstant.IMAGE_BASE_URL + responseJson.results.pro_thumb_path
        },
  ...
  static navigationOptions = ({ navigation, screenProps }) => ({
    ...
    headerTitleStyle: { color: '#A16E78', padding: Platform.OS === 'ios' ? 50 : 80 },
    headerStyle: {
      ...
      <TouchableWithoutFeedback style={{ flex: 1 }} onPress={Keyboard.dismiss} accessible={false}>
        <Content style={{ height: 100 }}>
          ...
          <View style={{ padding: 30, marginTop: 10 }}>
            ...
            <TouchableOpacity style={{ backgroundColor: '#A16E78', justifyContent: 'center', marginTop: 40, padding: 10, borderRadius: 10 }}
              onPress={this.artistEditProfile}>
...
AppRegistry.registerComponent('uGigs', () => ArtistEditProfile);